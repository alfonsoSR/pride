...
class Doppler(metaclass=ABCMeta):
    """Base class for implementation of Doppler calculation

    :param name: Unique name that identifies the Doppler model
    ...
    def __init__(self, exp: "Experiment") -> None:
        """Initialize doppler model from experiment"""
        ...
        # Ensure resources required to calculate the doppler
        log.debug(f"Ensuring resources for {self.name} doppler")
        self.ensure_resources()

        # Load resources
        log.debug(f"Loading resources for {self.name} doppler")
    ...
    def calculate_with_logging(self, obs: "Observation") -> Any:
        ...
        log.debug(
            f"Calculating {self.name} of {obs.source.name} from "
            f"{obs.station.name}"
        )
